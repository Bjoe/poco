...
void X509RevocationList::load(std::istream& istr)
{
	...
	_pCrl = PEM_read_bio_X509_CRL(pBIO, 0, 0, 0);
	BIO_free(pBIO);
...
void X509RevocationList::extractNames(std::string& cmnName, std::set<std::string>& domainNames) const
{
	domainNames.clear(); 
	if (STACK_OF(GENERAL_NAME)* names = static_cast<STACK_OF(GENERAL_NAME)*>(X509_CRL_get_ext_d2i(_pCrl, NID_subject_alt_name, 0, 0)))
    {
