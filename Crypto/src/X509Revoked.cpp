...
std::string X509Revoked::serialNumber() const
{
	...
	if (asn1serialNr)
	{
		BIGNUM* pBN = ASN1_INTEGER_to_BN(asn1serialNr, 0);
		if (pBN)
		{
...
X509Revoked::ReasonCode X509Revoked::getReasonCode() const
{
	...
	int index = -1;
	ASN1_ENUMERATED* reason = static_cast<ASN1_ENUMERATED*>(X509_REVOKED_get_ext_d2i(_pRevoked, NID_crl_reason, &index, NULL));
	if(!reason && (index == -1))
	{
		return result;
	}
